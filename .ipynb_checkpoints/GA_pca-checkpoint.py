...
def pca_arrows(
    ...
    # TODO: take into account if variable is well-explained. If not, make arrow and text alpha 0.5.
...
def full_outlier_pca(df, random_state=1, export=True, outname='pca'):
    ...
    values, loadings, contribs = generate_pca(
        df, random_state=random_state, export=True, exportname=f'{outname}_with_outliers.xlsx'
    )
    # Remove outliers
    df_noout = df.dropna()[~values['Outlier']]
    values2, loadings2, contribs2 =  generate_pca(
        df_noout, random_state=1, export=True, exportname=f'{outname}_without_outliers.xlsx'
    )
